...
// 32 reserved_word are checked in this function and return the
// result accordingly
bool isKeyword(char* str)
{
    ...
    for (int i = 0;
         i < sizeof(reserved_word) / sizeof(reserved_word[0]); i++) {
        if (strcmp(str, reserved_word[i]) == 0) {
            return true;
        }
    }
    return false;
...
{
    // Input 
    char lex_input[MAX_LENGTH] = "int x=y-z";
